...
for i in range(1644):
    ...
    for j in range(200):
        c.execute("INSERT INTO messages (id,in_id,user_id,random_id,message) VALUES (?,?,?,?,?)",(messages[j]['id'],nol,messages[j]['user_id'],messages[j]['random_id'],messages[j]['body']))
        conn.commit()
    if (i % 3) == 2:
        time.sleep(1)
c.close()
...
for i in range(1644):
    ...
    messages = response['items']
    print("обработано входящих ", i*200)
    for j in range(200):
        c.execute("INSERT INTO messages (id,out_id,user_id,random_id,message) VALUES (?,?,?,?,?)",(messages[j]['id'],nol,messages[j]['user_id'],messages[j]['random_id'],messages[j]['body']))
        conn.commit()
    if (i % 3) == 2:
        time.sleep(1)
c.close()
